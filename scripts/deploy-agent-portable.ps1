...
# agent identity, and cached policy files that PipeServer's ProtectFile handler reads AND writes
# while running WindowsIdentity.RunImpersonatedAsync as the calling interactive user - not SYSTEM,
# and not an elevated token even when that user is an Administrator, since Explorer/the shell context
# menu never runs elevated. Read-only (RX) isn't enough: the same impersonated context also has to
# create/update C:\ProgramData\CompanyDlp\Outbox\pending (AuditOutbox.EnqueueAsync),
# file-classification-cache.json, file-classification-status.json, and encrypted-file-hashes.json
# (FileClassificationCache/FileClassificationStatusStore/EncryptedFileHashStore) on every single
# classify/encrypt/decrypt - without write access those all fail with UnauthorizedAccessException
# under that same token, which is why classification results silently never persisted even after the
# earlier RX-only fix. Confirmed live. Modify (not just RX) here doesn't weaken anything real: DPAPI
# LocalMachine protection (not the file ACL) is what actually gates the plaintext secret in
# Credentials\*.bin, and any process on the machine can already call CryptUnprotectData with that
# scope regardless of this file's ACL - these are all local operational/cache files, not secrets.
Set-LockdownRecursive -Path $dataDir -ContainerGrants @("SYSTEM:(OI)(CI)F", "Administrators:(OI)(CI)F", "Users:(OI)(CI)M") -LeafGrants @("SYSTEM:F", "Administrators:F", "Users:M")
